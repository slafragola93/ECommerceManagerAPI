...
from src.schemas.dhl_shipment_schema import DhlCreateShipmentRequest, DhlCreateShipmentResponse
...
def get_dhl_shipment_service() -> IDhlShipmentService:
    """Dependency to get DHL shipment service"""
    configured_container = get_configured_container()
    return configured_container.resolve(IDhlShipmentService)


def get_dhl_tracking_service() -> IDhlTrackingService:
    """Dependency to get DHL tracking service"""
    configured_container = get_configured_container()
    return configured_container.resolve(IDhlTrackingService)


@router.post("/create", response_model=DhlCreateShipmentResponse)
async def create_shipment(
    request: DhlCreateShipmentRequest,
    ...
    Args:
        request: Richiesta di creazione spedizione con order_id
        dhl_service: Dipendenza del servizio di spedizione DHL
        
    Returns:
        Risposta di creazione spedizione con AWB, percorso label, etc.
        
    """
    try:
        logger.info(f"Creating DHL shipment for order {request.order_id}")
        
        result = await dhl_service.create_shipment(request.order_id)
...
async def get_tracking(
    tracking_numbers: str = Query(..., description="Comma-separated list of tracking numbers"),
