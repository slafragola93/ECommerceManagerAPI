...
class PlatformStateTriggerRepository(BaseRepository[PlatformStateTrigger, int], IPlatformStateTriggerRepository):
        ...
        try:
            return self._session.query(PlatformStateTrigger).filter(
                and_(
                    ...
                    PlatformStateTrigger.id_platform == id_platform,
                    PlatformStateTrigger.is_active == True
                )
            ).all()
        except Exception as e:
            raise InfrastructureException(
                f"Database error retrieving active triggers for event {event_type} and platform {id_platform}: {str(e)}"
            )
    ...
    def get_trigger_by_state(
        ...
        try:
            ...
            raise InfrastructureException(
                f"Database error retrieving trigger for event {event_type}, platform {id_platform}, "
