...
class ShipmentsHistory(Base):
    ...
    id_shipping = Column(Integer, ForeignKey('shippings.id'), nullable=False)
    id_shipping_state = Column(Integer, ForeignKey('shipping_states.id'), nullable=False)
    id_shipping_state_previous = Column(Integer, ForeignKey('shipping_states.id'), nullable=True)
