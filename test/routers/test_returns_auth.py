"""
Test semplificati per la funzionalità dei resi con autenticazione
"""
import requests
import json
...
TEST_USERNAME = "admin"  # Provo con credenziali più comuni
TEST_PASSWORD = "admin"
...
def test_partial_return_no_shipping():
    ...
    order_response = requests.get(f"{API_BASE}/orders/{order_id}", headers=headers)
    ...
    return_details = requests.get(f"{API_BASE}/returns/{return_id}", headers=headers)
...
def run_all_tests():
    """Esegue tutti i test"""
    print("=== INIZIO TEST RESI CON AUTENTICAZIONE ===")
